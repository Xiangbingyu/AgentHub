...
from app.llm.llm_executor import AgentExecutorFactory, FrameworkCliExecutor, InternalLlmExecutor, LlmExecutor
...
def test_framework_executor_parses_json_output(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.llm.llm_executor.os.name", "posix")
...
def test_framework_executor_passes_permission_flags(monkeypatch) -> None:
    runtime = RuntimeBundle(
        ...
        executor_policy={
            "kind": "framework_cli",
            "framework": "claude",
            "framework_allowed_tools": ["Read", "Edit", "Bash", "Write"],
...
def test_framework_executor_uses_powershell_wrapper_for_windows_claude(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.llm.llm_executor.os.name", "nt")
    monkeypatch.setattr("app.llm.llm_executor.subprocess.run", fake_run)
