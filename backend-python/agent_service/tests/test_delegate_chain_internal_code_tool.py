...
from agent_service.app.llm.llm_types import LlmResponse
...
def test_delegate_chain_internal_worker_uses_code_tool_to_write_workspace_file(monkeypatch) -> None:
    ...
    session = SessionRepository().create(
        SessionModel(
            ...
            title="delegate code session",
            status="active",
        )
    )

    target_relative_path = ".AgentHub/tests/internal-code-tool-example.py"
    target_absolute_path = Path(__file__).resolve().parents[2] / ".AgentHub" / "tests" / "internal-code-tool-example.py"
    ...
    monkeypatch.setattr("agent_service.app.tools.delegate_tool.DelegateTool._build_run_input_service", lambda self: service)
    ...
    subtasks = subtask_repository.list_by_parent_run_id(create_response.run_id)
    ...
    assert Path(orchestrator_plan.file_path).parent == Path(__file__).resolve().parents[2] / ".AgentHub" / "plans"
